...
var createScene = function () {
  ...
  BABYLON.SceneLoader.ImportMeshAsync("", "mesh/", "mainPost.glb").then(
    (result) => {
      ...
      signPlaneLeft.position = new BABYLON.Vector3(leftPost.position.x, 2.2, 0);
      signPlaneLeft.isVisible = false;
      signPlaneLeft.material = signmatDel;
      sings.push(signPlaneLeft);
      ...
      sturmankersVorderseite.push(leftStrVord, leftStrVordSraf);
      sturmankersRuckseite.push(leftStrRuck, leftStrRuckSraf);
  ...
  BABYLON.SceneLoader.ImportMeshAsync(
    ...
    //PLANE TO HOLD SIGN
    var signPlaneRight = BABYLON.MeshBuilder.CreatePlane("signPlaneRight", {
      height: 0.4,
      width: 0.4,
    });
    signPlaneRight.position = new BABYLON.Vector3(rightPost.position.x, 2.2, 0);
    signPlaneRight.material = signmatDel;
    signPlaneRight.isVisible = false;
    sings.push(signPlaneRight);

    for (let i = 0; i < sings.length; i++) {
      sings[i].actionManager = new BABYLON.ActionManager(scene);
      sings[i].actionManager.registerAction(
        new BABYLON.ExecuteCodeAction(
          BABYLON.ActionManager.OnPickUpTrigger,
          function () {
            leds[i].isVisible = false;
            sings[i].isVisible = false;
            ledsOn -= 1;
            ...
            if (ledsOn < 1) {
              ...
              currentActCol[5].children[2].innerHTML = "";
              ledParts[0].children[2].innerHTML = "&#10003";
              //change active color
              currentActCol[5].className = currentActCol[5].className.replace(
                " active-text-color",
                ""
              );
              ledParts[0].className += " active-text-color";
            }
          }
        )
      );
    ...
    sturmankersVorderseite.push(rightStrVord, rightStrVordSraf);
    sturmankersRuckseite.push(rightStrRuck, rightStrRuckSraf);
